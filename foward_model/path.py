...
def split(H_atm, P_atm, nlayer, layer_type=1, bottom_height=0.0, interp_type=1,
    ...
    Splits by equal changes in log pressure by default.

    Parameters
    ----------
    H_atm : ndarray
        Atmospheric model altitude grid; altitude H_atm[i] is at pressure level P_atm[i].
    P_atm : ndarray
        Atmospheric model pressure grid; pressure level P_atm[i] is at altitude H_atm[i].
    nlayer : int
    ...
    interp_type : int, default 1
        Interger specifying interpolation scheme. Linear interpolation by default.
...
def average(radius, H_atm, P_atm, T_atm, VMR_atm, ID, H_base, path_angle=0.0,
    ...
    Use absorber-amount-weighted averages by default.
